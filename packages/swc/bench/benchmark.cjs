...
async function run(extraction) {
  for (let i = 0; i < fixtures.length; i++) {
    await extraction(fixtures[i], { filename: `file-${i}.tsx`, importMap, entryPoint: true });
  }
}
...
(async () => {
  ...
  const a = await newExtraction(fixtures[0], { filename: "f.tsx", importMap, entryPoint: true });
  const b = await oldExtraction(fixtures[0], { filename: "f.tsx", importMap, entryPoint: true });
