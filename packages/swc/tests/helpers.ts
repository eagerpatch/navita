...
export function analyze(code: string): Analysis {
  const { program } = getParser().parseSync("out.tsx", code, { lang: "tsx", sourceType: "module" });
  ...
  const visit = (node: any) => {
    ...
    if (node.type === "CallExpression" && calleeName(node) === "collectResult") {
      ...
      if (thunk && (thunk.type === "ArrowFunctionExpression" || thunk.type === "FunctionExpression")) {
        const b = thunk.body;
        const expr = b.type === "BlockStatement" ? null : b;
        if (expr && expr.type === "CallExpression") resultCallee = calleeName(expr);
      }
      collectResults.push({
        ...
        sourceMap: { line: literalValue(sm.line), column: literalValue(sm.column) },
        resultCallee,
      });
