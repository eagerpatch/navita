...
export function printStyleBlocks(blocks: StyleBlock[]) {
  ...
  for (const style of blocks) {
    ...
    if (
      previousStyle &&
      previousStyle.container &&
      (!style.container || style.container !== previousStyle.container)
    ) {
      stylesheet += '}';
    }
    ...
    if (
      previousStyle &&
      previousStyle.support &&
      (!style.support || style.support !== previousStyle.support)
    ) {
      stylesheet += '}';
    }
    ...
    if (
      previousStyle &&
      previousStyle.media &&
      (!style.media || style.media !== previousStyle.media)
    ) {
      stylesheet += '}';
    }
