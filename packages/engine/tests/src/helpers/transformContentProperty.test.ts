...
describe("transformContentProperty", () => {
  ...
  it("should not add quotes to meaningful content values (examples from mdn: https://developer.mozilla.org/en-US/docs/Web/CSS/content)", () => {
    ...
    expect(
      transformContentProperty('url("http://www.example.com/test.png")'),
    ).toEqual('url("http://www.example.com/test.png")');
    expect(
      transformContentProperty("linear-gradient(#e66465, #9198e5)"),
    ).toEqual("linear-gradient(#e66465, #9198e5)");
    expect(
      transformContentProperty('image-set("image1x.png" 1x, "image2x.png" 2x)'),
    ).toEqual('image-set("image1x.png" 1x, "image2x.png" 2x)');
    expect(
    ...
    expect(transformContentProperty("counter(chapter_counter)")).toEqual(
      "counter(chapter_counter)",
    );
    expect(
      transformContentProperty("counter(chapter_counter, upper-roman)"),
    ).toEqual("counter(chapter_counter, upper-roman)");
    expect(transformContentProperty('counters(section_counter, ".")')).toEqual(
    ...
    ).toEqual('counters(section_counter, ".", decimal-leading-zero)');
    expect(transformContentProperty("attr(value string)")).toEqual(
      "attr(value string)",
    );
    ...
    expect(transformContentProperty("no-close-quote")).toEqual(
      "no-close-quote",
    );
    expect(
      transformContentProperty("open-quote counter(chapter_counter)"),
    ).toEqual("open-quote counter(chapter_counter)");
